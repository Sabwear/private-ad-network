...
function ChannelEditor({ channel }: { channel: ChannelManagementData["channels"][number] }) {
  ...
  return <details className="management-editor management-editor-wide channel-editor"><summary><Pencil size={13} /> Edit channel</summary><form action={action} className="management-inline-form"><header><strong>Edit channel</strong><small>Control delivery, the always-on broadcast clock, and every information layer shown over the stream.</small></header><input type="hidden" name="channelPublicId" value={channel.publicId} />{state.message ? <div className={`auth-message auth-message-${state.status}`}><ShieldCheck size={15} /><span>{state.message}</span></div> : null}<label><span>Channel availability</span><select name="status" defaultValue={channel.status}><option value="active">Viewer link available</option><option value="paused">Viewer link unavailable</option></select></label><label><span>Channel name</span><input name="name" minLength={2} maxLength={120} defaultValue={channel.name} required /></label><label><span>Description</span><textarea name="description" maxLength={300} rows={3} defaultValue={channel.description} /></label><ChannelDisplaySettingsFields settings={channel.settings} /><button className="button button-primary" type="submit" disabled={pending}>{pending ? <LoaderCircle className="auth-spinner" size={15} /> : <Pencil size={15} />}{pending ? "Saving..." : "Save channel and display"}</button></form></details>;
}
