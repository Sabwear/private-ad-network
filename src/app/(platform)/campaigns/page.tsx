...
import { CalendarDays, LockKeyhole, MapPinned, Plus, Target } from "lucide-react";
...
export default async function CampaignsPage() {
  ...
  const locations = workspace.permissions.canManageLocations ? await getLocationManagementData(workspace) : null;
  ...
  return <>
    <PageHeading eyebrow="Advertising" title="Campaigns" description="Configure campaign delivery, location inventory, targeting, approved media, dates, and controlled credit limits in one workflow." actions={<><span className={`data-source data-source-${result.source}`}>{result.source === "supabase" ? "Live data" : "Setup required"}</span>{result.canCreate ? <Link href="#create-campaign" className="button button-primary"><Plus size={16} /> Add campaign</Link> : null}</>} />
    <nav className="workspace-section-nav" aria-label="Campaign sections"><a href="#campaign-list"><Target size={16} /> Campaigns</a>{locations ? <a href="#locations"><MapPinned size={16} /> Locations & targeting</a> : null}</nav>
    ...
    {locations ? <section id="locations" className="workspace-section-anchor campaign-locations-section"><div className="section-heading"><div><p className="eyebrow">Delivery inventory</p><h2>Locations and targeting rules</h2><p>Configure venues, operating hours, service zones, traffic bands, and category exclusions used by campaign delivery.</p></div><span className="management-count"><MapPinned size={16} /> {locations.locations.length} locations</span></div><LocationManagement data={locations} fixedOrganizationId={workspace.organization.id} /></section> : null}
  </>;
}
