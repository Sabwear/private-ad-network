...
export type StreamingChannel = {
  ...
  items: Array<{ id: number; assetId: number; name: string; owner: string; position: number; hasHls: boolean; sourceType: "upload" | "youtube"; spendableCredits: number; baseRequiredCredits: number; funded: boolean }>;
};
...
export type ChannelManagementData = {
  ...
  availableMedia: Array<{ id: number; name: string; owner: string; hasHls: boolean; sourceType: "upload" | "youtube"; spendableCredits: number; baseRequiredCredits: number; funded: boolean }>;
};
...
export async function getChannelManagementData(): Promise<ChannelManagementData> {
  ...
  const [channelsResult, assignmentsResult, itemsResult, organizationsResult, mediaResult, walletsResult] = await Promise.all([
    ...
    supabase.from("organizations").select("id,display_name,status,ad_consumption_rate").order("display_name"),
    supabase.from("media_assets").select("id,name,organization_id,source_type,duration_ms,hls_master_storage_path,moderation_status,processing_status").eq("moderation_status", "approved").eq("processing_status", "ready").order("name"),
    supabase.from("wallets").select("organization_id,wallet_type,balance_projection").in("wallet_type", ["promotional", "earned", "purchased"]),
  ]);

  const error = channelsResult.error ?? assignmentsResult.error ?? itemsResult.error ?? organizationsResult.error ?? mediaResult.error ?? walletsResult.error;
  if (error) {
  ...
  const media = (mediaResult.data ?? []).map((asset) => ({
    ...
    spendableCredits: spendableByOrganization.get(asset.organization_id) ?? 0,
    baseRequiredCredits: Number(organizationsById.get(asset.organization_id)?.ad_consumption_rate ?? 0) * Math.max(asset.duration_ms ?? 15_000, 1_000) / 60_000,
    funded: (spendableByOrganization.get(asset.organization_id) ?? 0) + Number.EPSILON >= Number(organizationsById.get(asset.organization_id)?.ad_consumption_rate ?? 0) * Math.max(asset.duration_ms ?? 15_000, 1_000) / 60_000,
  }));
  ...
  const channels = (channelsResult.data ?? []).map((channel): StreamingChannel => ({
    ...
    items: (itemsResult.data ?? [])
      ...
      .flatMap((item) => {
        ...
        return asset ? [{ id: item.id, assetId: asset.id, name: asset.name, owner: asset.owner, position: item.position, hasHls: asset.hasHls, sourceType: asset.sourceType, spendableCredits: asset.spendableCredits, baseRequiredCredits: asset.baseRequiredCredits, funded: asset.funded }] : [];
      }),
  }));
