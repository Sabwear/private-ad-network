...
The dashboard header resolves the first channel visible to the signed-in user through a server-only repository. Search and guide navigation stay client-side, while the Live Beta URL is never hard-coded into the shell. The public stream permits anonymous playback with its bearer access key and is deliberately read-only. All video settings are persisted through the administrator-protected channel panel in Operations.

Browser channels use a persistent virtual broadcast timeline rather than starting a private playlist for each page view. Each channel stores whether broadcasting is enabled and a UTC broadcast epoch. The server returns that epoch, its current time, the ordered playlist, and authoritative asset durations; the player resolves `(server time - epoch) modulo loop duration` to select the active asset and seek offset. An access-key-protected, non-cacheable clock endpoint lets the player estimate server time at the request midpoint, avoiding dependence on the viewer device clock and compensating for page-load latency. It rechecks the shared position periodically and whenever the tab becomes visible. Enabling a broadcast, resuming an unavailable channel, or changing its playlist resets the epoch. This keeps all connected and late-joining browser viewers on the same pre-recorded loop without wasting bandwidth on a server process when nobody is watching.

The browser player has source-specific playback adapters behind that shared timeline. Private uploads use the authorized MP4/HLS delivery routes; YouTube references use a privacy-enhanced iframe with the JavaScript player protocol for seek, play, mute, and unmute commands. The timeline and overlay contract remain source-neutral. Saved channel settings control viewer actions, information layers, progress, overlay placement/treatment, banner placement, scaling, and a validated six-digit hex accent color. External URLs are normalized to canonical YouTube IDs at ingestion, and arbitrary iframe hosts are excluded by validation and Content Security Policy. Because YouTube availability and embedding policy are external dependencies, the submitted duration is reviewed before approval and a missing source follows the player's unavailable-source recovery path.
