...
Uploaded media jobs also generate adaptive 720p and 480p HLS renditions. The media library accepts either a private MP4 upload or a supported YouTube URL. Administrator submissions require a rights declaration and technical validation, then approve directly without a second review queue. Platform administrators manage continuous streams and all per-channel video settings from `/operations`, target businesses, add approved media, and copy or change the protected viewer URL. Live telemetry and accepted proof-of-play evidence are consolidated under `/monitor`. Each browser channel maintains one server-clock timeline, so every viewer joins the current point in the loop instead of restarting media on page load; an administrator can place that timeline on standby from Operations. The live player is read-only and applies the saved visibility, overlay, scaling, progress, banner, and accent settings. The first seeded stream is `Primary Network Channel`; the schema and interface support additional channels without changing the player contract.
